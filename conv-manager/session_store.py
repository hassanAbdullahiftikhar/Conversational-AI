...
class SessionStore:
    ...
    async def get_crm_profile_by_user(self, user_id: str) -> dict:
    ...
    async def update_crm_profile_by_user(self, user_id: str, key: str, value: Any) -> dict:
        async with _CRM_LOCK:
            async with aiosqlite.connect(str(_CRM_DB), isolation_level=None) as db:
                db.row_factory = aiosqlite.Row
                async with db.execute(
                ...
                now = time.time()
                await db.execute(
                    """INSERT INTO crm_profiles (user_id, profile_json, updated_at)
                       VALUES (?, ?, ?)
    ...
    async def delete_crm_key_by_user(self, user_id: str, key: str) -> bool:
        await self.update_crm_profile_by_user(user_id, key, None)
        return True
