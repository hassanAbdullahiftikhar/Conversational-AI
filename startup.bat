@echo off
rem Smart Home Assistant startup script for Windows
...
if not defined REBUILD_CORPUS goto skip_rebuild
echo.
...
if exist "conv-manager\smart_home_rag\repos\home-assistant.io" echo   home-assistant.io already exists, skipping.
if not exist "conv-manager\smart_home_rag\repos\home-assistant.io" git clone https://github.com/home-assistant/home-assistant.io.git conv-manager\smart_home_rag\repos\home-assistant.io

if exist "conv-manager\smart_home_rag\repos\zigbee2mqtt.io" echo   zigbee2mqtt.io already exists, skipping.
if not exist "conv-manager\smart_home_rag\repos\zigbee2mqtt.io" git clone https://github.com/Koenkk/zigbee2mqtt.io.git conv-manager\smart_home_rag\repos\zigbee2mqtt.io

if exist "conv-manager\smart_home_rag\repos\esphome-docs" echo   esphome-docs already exists, skipping.
if not exist "conv-manager\smart_home_rag\repos\esphome-docs" git clone https://github.com/esphome/esphome-docs.git conv-manager\smart_home_rag\repos\esphome-docs
...
if not "%errorlevel%"=="0" goto no_gpu
...
set DL_MB=0
for /f %%p in ('docker exec conversational-ai-llm-engine-1 sh -lc "du -sm /root/.cache/huggingface/hub/models--unsloth--gemma-4-E4B-it-GGUF/blobs 2>/dev/null ^| cut -f1" 2^>nul') do set DL_MB=%%p
